...
@Service
public class StatusEmplService {

    private final StatusEmplRepository repository;

    public StatusEmplService( StatusEmplRepository repository) {
    ...
    private Pageable createPageable( int page, int size, String sortParam) {
        ...
        // ожидаем формат "field,dir" например "status,asc"
    ...
    // Поиск по status (опционально)
    public Page searchByStatus( String status, int page, int size, String sort) {
        Pageable pageable = createPageable(page, size, sort);
        return repository.findByStatusContainsIgnoreCase(status, pageable);
    }
